...
- Do not run long browser e2e by default. Prefer focused package tests and keyless snapshots; run only the exact e2e scenario that covers the changed acceptance path when lower-level checks cannot prove it.
