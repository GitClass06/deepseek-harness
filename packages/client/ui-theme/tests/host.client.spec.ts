...
describe('ui-theme host', () => {
  it('registers, validates, and disposes the durable theme namespace with its fiber', async () => {
    ...
    await ctx.settings.update(ns, { preference: 'national-day' })
    expect(ctx.settings.get(ns)).toEqual({ preference: 'national-day' })
  ...
  it('answers each collection with the current durable preference until disposal', async () => {
    ...
    await ctx.settings.update(settingsNamespace(THEME_SETTINGS_NAMESPACE), { preference: 'national-day' })
    expect(scriptText(collect(ctx)[0])).toContain('const preference = "national-day"')
    await fiber.dispose()
