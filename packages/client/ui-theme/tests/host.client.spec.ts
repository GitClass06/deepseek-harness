...
describe('ui-theme host', () => {
  it('registers, validates, and disposes the durable theme namespace with its fiber', async () => {
    ...
    await ctx.settings.update(ns, { preference: 'mid-autumn' })
    expect(ctx.settings.get(ns)).toEqual({ preference: 'mid-autumn' })
  ...
  it('answers each collection with the current durable preference until disposal', async () => {
    ...
    await ctx.settings.update(settingsNamespace(THEME_SETTINGS_NAMESPACE), { preference: 'spring-festival' })
    expect(scriptText(collect(ctx)[0])).toContain('const preference = "spring-festival"')
    await fiber.dispose()
