...
The modal panel resets inherited label and nav-fill tokens to `--dsw-specific-settings-panel-*` values before rendering its sections. This keeps a theme that retints `sidebar.settings` readable inside the fixed panel while leaving trigger placement, panel dimensions, and section layout unchanged.
